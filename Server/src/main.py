...
def setup_file_logging(log_path: str | None = None):
    ...
    try:
        if not log_path:
            log_path = os.environ.get("UNITY_MCP_LOG_PATH")
        ...
        # Clear existing file handlers to allow reconfiguration
        ...
        try:
            ...
            # Clear existing file handlers for telemetry too
        ...
        logger.info(f"File logging configured at: {log_path}")
    except Exception as exc:
        # Never let logging setup break startup
...
Examples:
  ...
  # Use custom log path
  python -m src.server --logpath "/path/to/my.log"
  ...
  UNITY_MCP_TRANSPORT=http UNITY_MCP_HTTP_URL=http://localhost:9000 python -m src.server
    ...
    parser.add_argument(
        ...
        "Can also set via UNITY_MCP_LOG_PATH environment variable.",
    )

    args = parser.parse_args()

    # Configure file logging
    config.log_path = args.logpath or os.environ.get("UNITY_MCP_LOG_PATH")
    ...
    os.environ["UNITY_MCP_HTTP_HOST"] = str(http_host)
