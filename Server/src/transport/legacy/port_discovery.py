...
- Unity now writes a per-project port file named like
  `<status-dir>/unity-mcp-port-<hash>.json` to avoid projects overwriting
  each other's saved port. The legacy file `unity-mcp-port.json` may still exist.
  The status dir is resolved from `UNITY_MCP_STATUS_DIR` (if set), otherwise it
  defaults to a project-scoped directory when possible.
...
class PortDiscovery:
    ...
    def get_registry_dir() -> Path:
        ...
        # Default to a project-scoped directory when the server is launched
        # from a Unity project root (contains Assets/ and ProjectSettings/).
        try:
            cwd = Path.cwd()
            if (cwd / "Assets").exists() and (cwd / "ProjectSettings").exists():
                return cwd / ".unity-mcp"
        except Exception:
            pass
    ...
    def discover_all_unity_instances() -> list[UnityInstanceInfo]:
        ...
        for status_file_path in status_files:
            try:
                ...
                hash_value = filename.replace("unity-mcp-status-", "").replace(
                    ".json", ""
                )
